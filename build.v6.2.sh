...
wget -q https://registry.hub.docker.com/v2/repositories/willh/ngcli/tags -O -   | jq '[.results[].name]' > "$DockerHubTags"

npm view @angular/cli versions --json \
    | jq '[.[] | select(. | startswith("6.2"))]' \
...
while read n; do
    ...
    then
        ...
        echo --------------------------------------------------------------
        docker build -t willh/ngcli:$v --build-arg CLI_VERSION=$v -f Dockerfile.v6.2 .
        echo --------------------------------------------------------------
